---
name: test-patterns
description: テスト設計パターン集。RedAgentで利用するVitest/Jest/RTLパターン、Mock例、包括的なテスト実装例をまとめたもの。
---
...
すべてのテストファイルは、**サイズプレフィックス**をファイル名に付けなければならない。

```text
ComponentName.small.test.ts
ComponentName.medium.test.ts
ComponentName.large.test.ts
```

- `ComponentName.test.ts` — **使用不可**（サイズプレフィックスがない）
- `ComponentName.spec.ts` — **使用不可**（`.test.ts` を使用する）

## When to Write Tests

- **変更することが怖い**と感じるときにテストを書く（予測可能な変更容易性：「この変更で何か壊れないか？」）。
- TDDサイクルを通じて**設計を学び、改善したい**ときにテストを書く。
- カバレッジのためだけにテストを書かない。テストが不安を減らしたり設計を改善したりしないのであれば、そのテストに価値はない。
- TDDにおけるリファクタリングとは、**拡張には開かれ、変更には閉じる（OCP）**というバランスを継続的に調整するプロセスである。
...
仕様に記載されているすべてのバリデーションルールとエラーケースをテストする。
...
類似する入力をグループ化し、各グループから代表的な入力を1つテストする。
...
複数の入力条件を持つ機能では、デシジョンテーブルを用いてテストする。
...
これらのテストを `npm test -- CreateApp.test.ts` で実行した場合、**`CreateAppInteractor` がまだ存在しないため、すべてのテストはFAILしなければならない。**
