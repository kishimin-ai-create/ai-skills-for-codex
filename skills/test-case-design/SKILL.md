...
5. 未実装ケースは`describe`/`it`の名前で意図を表す`test.todo`にする。各`test.todo`の直前に、`// ID`、`// Source`、`// Arrange`、`// Act`、`// Assert`、必要な`// Error`または`// Blocked by`、`// Priority`をこの順で書く。コメントを`describe`の先頭やファイル末尾へまとめない。todoを成功扱いにしない。
...
- `Arrange`、`Act`、`Assert`は実装詳細ではなく、入力条件、利用者の操作、観測可能な結果を書く。
