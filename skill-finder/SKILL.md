---
name: skill-finder
description: 'Use only when the user explicitly asks to discover or evaluate a Skill outside this PC''s local catalog. Complementary to skill-selector; do not auto-invoke for routine work. Apply a mandatory isolated evaluation before adoption.'
---
...
- The user did not ask for discovery → as a meta-skill, this never auto-fires (cf. the applicable user-level AGENTS.md skill policy).
...
4. **構造監査（推奨）。** 候補Skillのfrontmatter、本文長、参照リンク、不審なスクリプト、隠れたUnicode命令をローカルで確認する。監査エラーの候補は評価前に棄却理由を記録する。
...
7. **Fork-and-fix path.** If a candidate is close-but-not-quite, preferコピーをこのPCの`$HOME/.agents/<name>`へ保存して修正し、呼び出し側で回避しない。原文との差分を記録する。
...
- **VoltAgent/awesome-agent-skills**: README is org-grouped (Anthropic / Stripe / HashiCorp / Cloudflare / Sentry / etc.). Extract entries with:
  ...
  ```bash
  Use the repository's documented search interface and save the exact candidate reference.
  ```
